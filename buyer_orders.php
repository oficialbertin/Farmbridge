...
<main class="container mt-5">
    ...
    <div id="orders-list">
        <?php
        $result = $conn->query("SELECT o.*, c.name AS crop_name, u.name AS farmer_name FROM orders o JOIN crops c ON o.crop_id = c.id JOIN users u ON c.farmer_id = u.id WHERE o.buyer_id = $buyer_id ORDER BY o.created_at DESC");
        if ($result && $result->num_rows > 0) {
            echo '<div class="table-responsive"><table class="table table-bordered table-striped" id="ordersTable">';
            echo '<thead><tr><th>Order ID</th><th>Crop</th><th>Farmer</th><th>Quantity</th><th>Status</th><th>Total (RWF)</th><th>Ordered At</th><th>Actions</th></tr></thead><tbody>';
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<td>';
                echo '<a href="order_details.php?id=' . $row['id'] . '" class="btn btn-sm btn-primary mb-1"><i class="bi bi-eye"></i> View</a> ';
                if (in_array($row['status'], ['paid', 'completed'])) {
                    echo '<a href="order_details.php?id=' . $row['id'] . '#raiseDispute" class="btn btn-sm btn-warning"><i class="bi bi-exclamation-diamond"></i> Raise Dispute</a>';
                }
                echo '</td>';
                echo '</tr>';
            }
