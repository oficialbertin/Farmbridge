...
// Get crop data
$stmt = $conn->prepare("SELECT crops.*, users.name AS farmer_name FROM crops JOIN users ON crops.farmer_id = users.id WHERE crops.id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $update_stmt = $conn->prepare("
            ...
            WHERE id = ?
        ");
        $update_stmt->bind_param("ssissssssi", 
                ...
                <div class="card-body">
                    ...
                    <form method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <p class="text-muted"><strong>Crop ID:</strong> <?= $crop['id'] ?></p>
                                    <p class="text-muted"><strong>Farmer:</strong> <?= htmlspecialchars($crop['farmer_name']) ?></p>
                                </div>
                                <div class="col-md-6">
                                    <p class="text-muted"><strong>Listed:</strong> <?= date('M d, Y', strtotime($crop['listed_at'])) ?></p>
                                </div>
                            </div>
                        </div>
